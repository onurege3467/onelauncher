...
enum BindError {
    /// Only Windows reports a held endpoint; the unix path clears a stale
    /// socket and rebinds instead of surfacing this
    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
    ...
    let reply = match IpcCommand::decode(&line) {
        Some(command) => match commands.send(command) {
            Ok(()) => format!("{REPLY_OK}\n"),
            Err(_) => "ERR the launcher is shutting down\n".to_string(),
        },
...
mod imp {
    ...
    pub async fn serve(listener: Listener, commands: UnboundedSender<IpcCommand>) {
        ...
        loop {
            ...
            tokio::spawn(async move { handle(connected, &commands).await });
        }
    }
}
...
mod imp {
    ...
    fn endpoint() -> Option<PathBuf> {
        oneclient_common::paths::config_dir()
            .ok()
    ...
    pub async fn bind() -> Result<Listener, BindError> {
        ...
        match bound(&path) {
            ...
            Err(err) if err.kind() == std::io::ErrorKind::AddrInUse => {
                ...
                bound(&path).map_err(BindError::Io)
            }
            Err(err) => Err(BindError::Io(err)),
        }
    }

    /// The launcher directory is not private, so the endpoint itself has to
    /// keep other local users from sending a launch of their choosing
    fn bound(path: &Path) -> std::io::Result<Listener> {
    ...
    pub async fn serve(listener: Listener, commands: UnboundedSender<IpcCommand>) {
        loop {
            match listener.listener.accept().await {
                Ok((stream, _)) => {
                    let commands = commands.clone();
                    tokio::spawn(async move { handle(stream, &commands).await });
                }
                Err(_) => return,
            }
        }
